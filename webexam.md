...
1. Форма - это раздел документа, предназначенный для ввода и отправки данных пользователем на сервер. 
    - `action` - действие
    - `method` - http-метод
    - `name` - имя формы
    - `target` - цель
    - `enctype` - вид шифрования
    - `autocomplete` - автозаполнение, имеет параметры on и off
    - `novalidate` - отменяет проверку перед отправкой
2. background-color.
```css
    .element {
